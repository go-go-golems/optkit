...
RelatedFiles:
    ...
    - Path: repo://optkit/ttmp/2026/08/26/OPTKIT-020--asset-variable-proof-for-representation-prompts/design-doc/01-intern-guide-to-artifact-valued-prompt-variables-and-bounded-previews.md
      Note: Primary design deliverable whose research and delivery this diary records
    - Path: repo://optkit/ttmp/2026/08/26/OPTKIT-020--asset-variable-proof-for-representation-prompts/various/remarkable-dry-run.log
      Note: Ticket bundle selection and destination dry-run evidence
    - Path: repo://optkit/ttmp/2026/08/26/OPTKIT-020--asset-variable-proof-for-representation-prompts/various/remarkable-upload.log
      Note: Successful rendered PDF upload receipt
ExternalSources: []
Summary: Chronological research, design, implementation, validation, and delivery record for OPTKIT-020.
LastUpdated: 2026-08-26T14:20:30.821776637-04:00
WhatFor: ""
WhenToUse: ""
---
...
Design the artifact-valued summary-prompt mutation and bounded one-chunk preview. This diary records research, design decisions, validation, commits, delivery evidence, failures, and continuation instructions.
...
**Assistant interpretation:** Generalize the scalar workbench to sensitive, content-addressed, expensive configuration without prompt text in patch metadata or client-side fake generation.

**Inferred user intent:** Prove one core workflow supports both scalar and asset coordinates honestly.
...
- The semantic fixture may expose only precomputed representation text; implementation must find or build an honest producer instead of editing recorded output and calling it generation.
...
- Review prompt leakage, provisional/stored digest parity, provider/model identity, preview evidence labeling, and resource bounds.
- Review all proposed public schema/API names before implementation makes them expensive to change.

### What should be done in the future

- Inventory/prove the producer first; only then implement config/variable, provisional assets, bounded preview, sealing, and editor conformance.
...
- `rag-ttc/pkg/ttc/search/types.go`
- `rag-ttc/apps/specialist/web/src/components/Artifact.tsx`
- `rag-ttc/apps/specialist/web/src/layerwidgets/index.tsx`

Validate the design workspace with:

```bash
docmgr validate frontmatter --doc optkit/ttmp/2026/08/26/OPTKIT-020--asset-variable-proof-for-representation-prompts/reference/01-implementation-diary.md
docmgr doctor --ticket OPTKIT-020 --stale-after 30
```
...
- Ran `docmgr doctor --ticket OPTKIT-020 --stale-after 30` and obtained `All checks passed`.
...
- Ran the program upload script in `--dry-run` mode, then rendered and uploaded `OPTKIT-020 Prompt Asset Variable Guide.pdf`.
...
- `remarquee` reported: `OK: uploaded OPTKIT-020 Prompt Asset Variable Guide.pdf -> /ai/2026/08/26/OPTKIT-020`.
...
- Validate locally with `docmgr doctor --ticket OPTKIT-020 --stale-after 30`.
- Consult `tasks.md` for the still-open implementation sequence.

### Technical details

```text
bundle: OPTKIT-020 Prompt Asset Variable Guide.pdf
remote: /ai/2026/08/26/OPTKIT-020
